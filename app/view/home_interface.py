...
class BannerWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.galleryLabel = QLabel('ATRI-YOLO', self)
...
class HomeInterface(ScrollArea):
    ...
    def __initWidget(self):
        ...
        # # 增加实验室的logo显示
        # self.labLogo = ImageLabel(self)
        # logo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'resource', 'images', 'MMDL-logo.png'))
        # if os.path.exists(logo_path):
        #     pixmap = QPixmap(logo_path)
        #     if not pixmap.isNull():
        #         # 设置 logo 的大小为原始大小的一半
        #         scaled_size = QSize(int(1170 / 3), int(912 / 3))
        #         scaled_pixmap = pixmap.scaled(scaled_size, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        #         self.labLogo.setPixmap(scaled_pixmap)
        #         self.labLogo.setFixedSize(scaled_size)
        #         logoLayout.addWidget(self.labLogo)
        #     else:
        #         QMessageBox.warning(self, "警告", f"无法加载图片: {logo_path}")
        # else:
        #     QMessageBox.warning(self, "警告", f"找不到图片文件: {logo_path}")

        # 将 logo 布局添加到主布局中
        self.vBoxLayout.addLayout(logoLayout)
        
        # 添加一个弹性空间，使 logo 在垂直方向上居中
